...
int main(int agrc, char *argv[]) {
    ...
    cudaMalloc ((void **) &H, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &F, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &u, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &v, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &h, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &u_save, sizeof(double)*(nx*ny*noutput));
    cudaMalloc ((void **) &v_save, sizeof(double)*(nx*ny*noutput));
    cudaMalloc ((void **) &h_save, sizeof(double)*(nx*ny*noutput));
    cudaMalloc ((void **) &u_accel, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &v_accel, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &uh, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &vh, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Ux, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Uy, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Vx, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Vy, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &h_mid_xt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &h_mid_yt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &uh_mid_xt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &uh_mid_yt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &vh_mid_xt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &vh_mid_yt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Ux_mid_xt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Uy_mid_yt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Vx_mid_xt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &Vy_mid_yt, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &h_new, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &uh_new, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &vh_new, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &u_new, sizeof(double)*(nx*ny));
    cudaMalloc ((void **) &v_new, sizeof(double)*(nx*ny));

    // copy data from cpu to gpu
    cudaMemcpy(H, H_cpu, sizeof(double)*(nx*ny), cudaMemcpyHostToDevice);
    cudaMemcpy(F, F_cpu, sizeof(double)*(nx*ny), cudaMemcpyHostToDevice);
    cudaMemcpy(u, u_cpu, sizeof(double)*(nx*ny), cudaMemcpyHostToDevice);
    cudaMemcpy(v, v_cpu, sizeof(double)*(nx*ny), cudaMemcpyHostToDevice);
    cudaMemcpy(h, h_cpu, sizeof(double)*(nx*ny), cudaMemcpyHostToDevice);
    ...
    for (int n = 0; n < nt; ++n) {
        if (n % timesteps_between_outputs == 0) {
            ...
            store_data<<<dimGrid, dimBlock>>>(nx, ny, u, v, h, u_save, v_save, h_save, index);
            ++i_save;
        }
        cudaDeviceSynchronize();
        compute_accleration<<<dimGrid, dimBlock>>>(u_accel, v_accel, F, u, v, g, dx, dy, H, nx, ny);
        lax_wendroff_step1<<<dimGrid, dimBlock>>>(nx, ny, uh, u, h, vh, v, Ux, g, Uy, Vx, Vy);
        cudaDeviceSynchronize();
        lax_wendroff_step2<<<dimGrid, dimBlock>>>(nx, ny, uh, u, h, vh, v, Ux, g, Uy, Vx, Vy, h_mid_xt, uh_mid_xt, vh_mid_xt, h_mid_yt, uh_mid_yt, vh_mid_yt, Ux_mid_xt, Uy_mid_yt, Vx_mid_xt, Vy_mid_yt, dx, dy, dt);
        cudaDeviceSynchronize();
        lax_wendroff_step3<<<dimGrid, dimBlock>>>(nx, ny, u, v, h, u_accel, v_accel, uh, vh, h_mid_xt, uh_mid_xt, vh_mid_xt, h_mid_yt, uh_mid_yt, vh_mid_yt, Ux_mid_xt, Uy_mid_yt, Vx_mid_xt, Vy_mid_yt,dx, dy, dt, h_new, u_new, v_new);
        cudaDeviceSynchronize();
        update_boudary<<<dimGrid, dimBlock>>>(nx, ny, u, v, h, h_new, u_new, v_new);
        cudaDeviceSynchronize();
    }
    // copy data from gpu to cpu
    cudaMemcpy(u_save_cpu, u_save, sizeof(double)*(nx*ny*noutput), cudaMemcpyDeviceToHost);
    cudaMemcpy(v_save_cpu, v_save, sizeof(double)*(nx*ny*noutput), cudaMemcpyDeviceToHost);
    cudaMemcpy(h_save_cpu, h_save, sizeof(double)*(nx*ny*noutput), cudaMemcpyDeviceToHost);
